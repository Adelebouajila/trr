...
app.post('/api/bookings', async (req, res) => {
  try {
    ...
    console.log("Tour builder data check:", {
      hasTourBuilderData: !!req.body.tourBuilderData,
      tourBuilderDataType: typeof req.body.tourBuilderData,
      tourBuilderDataSize: req.body.tourBuilderData ? JSON.stringify(req.body.tourBuilderData).length : 0
    });
    ...
    try {
      ...
      try {
        ...
        if (fs.existsSync(enhancedGeneratorPath)) {
          ...
          // Add tour builder data to the booking data for email generation
          const enhancedBookingData = {
            ...result.rows[0],
            tourBuilderData: req.body.tourBuilderData || null
          };
          console.log("Enhanced booking data for email:", {
            hasTourBuilderData: !!enhancedBookingData.tourBuilderData,
            tourBuilderDataType: typeof enhancedBookingData.tourBuilderData
          });
          emailHtml = enhancedGenerator.generateEnhancedBookingEmail(enhancedBookingData, tourDetails);
          ...
          emailHtml = generateBookingEmail(enhancedBookingData, tourDetails);
...
// PHP Page Generation API
app.post('/api/generate-php-page', async (req, res) => {
  console.log('📋 PHP Generation Request Data:', JSON.stringify(req.body, null, 2));
  try {
    if (!req.user || (req.user.role !== 'admin' && req.user.is_admin !== true)) {
      return res.status(403).json({ error: 'Admin access required' });
    }

    const { tourId, itinerary, notSuitableFor, fullDescription, gygReviews, tripadvisorReviews, faq, paymentLink, urlSlug } = req.body;

    // Get tour details
    const tourResult = await pool.query('SELECT * FROM tours WHERE id = $1', [tourId]);
    ...
    // Process arrays if they're PostgreSQL format
    const processArray = (field) => {
      if (Array.isArray(field)) return field;
      if (typeof field === 'string' && field.startsWith('{') && field.endsWith('}')) {
        try {
          const arrayString = field.substring(1, field.length - 1);
    ...
    const processedTour = {
      ...
      languages: processArray(tour.languages),
      highlights: processArray(tour.highlights),
      included: processArray(tour.included),
      notIncluded: processArray(tour.notIncluded)
    };

    // Generate PHP page using the fixed generator
    const phpPageGenerator = require('./enhanced-php-generator-fixed');
    const generator = new phpPageGenerator();
    ...
    const filename = await generator.generateTourPage(pageData);
    
    res.json({ 
      success: true, 
      filename, 
      message: 'PHP page generated successfully' 
    });
...
const frenchTranslation = require('./api/french-translation');

// GET /api/get-php-files
app.get('/api/get-php-files', frenchTranslation.getPhpFiles);

// POST /api/parse-php-content
app.post('/api/parse-php-content', frenchTranslation.parsePhpContent);

// POST /api/generate-french-page
app.post('/api/generate-french-page', frenchTranslation.generateFrenchPage);
